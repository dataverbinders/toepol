...
class SparkCluster:
    def setup(self) -> SparkSession:
        ...
        spark.conf.set('spark.sql.parquet.datetimeRebaseModeInWrite',
                       'CORRECTED')
...
def download_bag_zip(bag_url=BAG_URL, bag_file=BAG_FILE):
...
def unzip_bag_data(bag_file=BAG_FILE, data_dir=DATA_DIR):
    ...
    #  for name in zipnames:
        #  with ZipFile(f'{data_dir}/{name}') as zip:
            ...
            #  files = [f'{DATA_DIR}/{target_folder}/{f}'
...
def convert_vlak(df, spark, transformer):
    ...
    new_df = spark.createDataFrame(new_polygons, ['id', 'voorkomen', 'geo_polygon'])
    cond = [
        df['Objecten:identificatie']['_VALUE'] == new_df.id,
        df['Objecten:voorkomen']['Historie:Voorkomen']['Historie:voorkomenidentificatie'] == new_df.voorkomen
    ]
    ...
    for row in df.collect():
        ...
        if row['Objecten:geometrie']['gml:Polygon'] is not None:
            ...
            if interiors is not None:
                for interior in interiors:
                    positions = interior['gml:LinearRing']['gml:posList']['_VALUE']
                    interior_lr = create_linear_ring(positions, transformer, dimension)
                    interior_lrs.append(interior_lr)
...
def store_df_as_parquet(df, object_type):
    target_file = f'{DATA_DIR}/{object_type}.parquet'
    df.repartition(1) \
        .write \
        .mode('overwrite') \
        .parquet(target_file)
...
with Flow('bag2gcs') as flow:
    ...
    if DOWNLOAD_BAG:
        bag_file = download_bag_zip()
    else:
        bag_file = BAG_FILE

    zip_dict, lev_doc = unzip_bag_data(bag_file)

    with SparkCluster() as spark:
        for key, val in object_map.items():
            target_dir = f'{DATA_DIR}/{key}'
            files = unzip_sub_file(zip_dict[key], target_dir)
            df = create_spark_df(spark, val, files)
            df = convert_geometry(df, spark)
            parquet_file = store_df_as_parquet(df, val)
            write_parquet_to_gcs(parquet_file)
